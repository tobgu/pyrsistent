...
def test_predicate():
    ...
    assert m.transform(["foo", lambda x: x.startswith("b"), "baz"], inc) == {
        "foo": {"bar": {"baz": 2}, "qux": {"baz": 1}}
    }
...
def test_key_value_predicate():
    ...
    assert m.transform([lambda k, v: (k, v) == ("foo", 1),], lambda v: v * 3) == {
        "foo": 3,
        "bar": 2,
    }
...
def test_discard_multiple_elements_in_pvector():
    assert freeze([0, 1, 2, 3, 4]).transform([lambda i: i % 2], discard) == freeze(
        [0, 2, 4]
    )
