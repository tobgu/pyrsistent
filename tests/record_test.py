    ...
    h = Hierarchy.create(
        {
            ...
            "point2": {"x": 1, "y": "foo", "extra_field_1": "extra_data_1"},
            "points": [
                {"x": 1, "y": "foo", "extra_field_2": "extra_data_2"},
...
def test_nested_create_serialize():
    node = Node(
        applications=[
            Application(name="myapp", image="myimage"),
...
def test_pset_field_name():
    ...
    assert (Record().value.__class__.__name__, Record().value2.__class__.__name__) == (
        "SomethingPSet",
        "IntPSet",
    )
...
def test_pset_multiple_types_field_name():
    ...
    assert Record().value.__class__.__name__ == "SomethingIntPSet"
...
def test_pvector_multiple_types_field_name():
    ...
    assert Record().value.__class__.__name__ == "SomethingIntPVector"
...
def test_pmap_field_invariant():
    ...
    class Record(PRecord):
        value = pmap_field(
            int,
            int,
            invariant=(lambda pmap: (len(pmap) == 1, "Exactly one item required.")),
        )
